...
class LogAnalyzer:
    ...
    def parse_ufw(self):
        ...
        with open(self.ufw_log_path, 'r') as file:
            for line in file:
                # Extract date, time, SRC (source IP), DST (destination IP), and PROTO (protocol)
                match = re.search(r'(\w+\s+\d+\s+\d+:\d+:\d+)\s+[\w-]+\s+kernel:.*SRC=(\S+)\s+DST=(\S+)\s+PROTO=(\S+)', line)
                if match:
                    date_time, src_ip, dst_ip, protocol = match.groups()
                    ufw_data.append([date_time, src_ip, dst_ip, protocol])
    
        # Convert to DataFrame
        ufw_df = pd.DataFrame(ufw_data, columns=['datetime', 'src_ip', 'dst_ip', 'protocol'])
        ufw_df['datetime'] = pd.to_datetime(ufw_df['datetime'])
        return ufw_df
